...
func (h *StudentController) UpsertMyProfile(c *gin.Context) {
    ...
    // Supporting documents (transcript / resume / schedule) live in their own
    // table; upsert them alongside the profile. Best-effort — a failure here
    // shouldn't fail a profile save that already committed.
...
func (h *StudentController) findByUserID(userID uint) (*models.Student, error) {
    ...
    err := h.db.Preload("User").Preload("AttachmentStudent").Where("user_id = ?", userID).First(&student).Error
    if err != nil {
...
func mapStudentToResponse(student *models.Student) *dto.StudentProfileResponse {
    ...
    transcript, resume, schedule := "", "", ""
    if student.AttachmentStudent != nil {
        transcript = student.AttachmentStudent.Transcript
        resume = student.AttachmentStudent.Resume
        schedule = student.AttachmentStudent.Schedule
    }
