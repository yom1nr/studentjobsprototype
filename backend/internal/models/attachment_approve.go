...
type Approve struct {
	...
	Status       string    `gorm:"size:50;not null;default:'pending'" json:"status"` // pending | approved | rejected | request_document

	// RequestNote is the text the admin sends when status = "request_document"
	// (which documents / what to fix). RequestNoteAckAt is set when the employer
	// marks it read; cleared on each new request.
	RequestNote      string     `gorm:"type:text" json:"request_note"`
	RequestNoteAckAt *time.Time `json:"request_note_ack_at"`
