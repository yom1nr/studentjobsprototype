...
export function UploadCard({ label, camera, onUpload, value }: Readonly<{ label: string; camera?: boolean, onUpload?: (url: string) => void, value?: string }>) {
  ...
  return (
    <Box>
      ...
      >
        <input type="file" accept=".pdf,.jpg,.jpeg,.png" hidden disabled={uploading} onChange={(e) => void handleFileChange(e)} />
        {uploading ? (
        ...
        {!uploading && !value && <Typography sx={{ fontSize: 11, color: '#9AA0A6' }}>หรือ ลากไฟล์มาวางที่นี่</Typography>}
        {!uploading && !value && <Typography sx={{ fontSize: 11, color: '#9AA0A6' }}>รองรับไฟล์ PDF, JPG, PNG (ขนาดไม่เกิน 5MB)</Typography>}
